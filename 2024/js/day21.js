...
function part2notMy(inp) {
  ...
  function translateInstruction(key) {
    if (key[0] === key[1]) {
      return "A";
    } else {
  ...
  class RobotKeypad {
    ...
    #translateParentInstructions(instructions) {
      ...
      for (let instruction of instructions.keys()) {
        ...
        this.#setNewKeystrokes(
          commandCounts,
          "A" + translateInstruction(instruction),
          count,
        );
      }
    ...
    #translateMainKeypadInstructions(layers) {
      ...
      for (let i = 0; i < instructions.length - 1; i++) {
        this.#setNewKeystrokes(
          commandCounts,
          "A" + translateInstruction(instructions.substring(i, i + 2)),
        );
      }
      for (; layers > 1; layers--) {
    ...
    get complexity() {
      ...
      return sum(this.#instructions.values()) * +this.#code.slice(0, 3);
    }
  }
  ...
  function getCodeComplexities(input, layers) {
    ...
    for (let line of input) {
      let keypad = new RobotKeypad(line, layers);
